...
test('clickSortFrom', async ({ page }) => {
    // Open the website
    await page.goto("http://the-internet.herokuapp.com/tables");
    await page.waitForTimeout(1000);
    //await page.click("//th[@class='header headerSortUp']");
    //await page.click("//th[@class='header headerSortUp']");
    for (let x = 0; x < 2; x++) {
        await page.waitForTimeout(500);
        await page.locator("(//span[contains(.,'Due')])[1]").click();  // klikni po jacheike

    }

    let itemsPrice = await page.$$('(//table[@id="table1"]//tbody/tr/td[4])');
...
//npx playwright test tests/33.spec.js --headed --project chromium